...
- Basic design 003_BD (`docs/en/010_basic-design/003/003_BD_ダッシュボード.md`): SCR-003 at `/`, one section per widget, PC and SP layouts, the chart approach and each chart's accessible text or table equivalent, and the screen transition from login.
- Database design 003_DB (`docs/en/database/003/003_DB_ダッシュボード.md`): the `completed_at_utc` column and its constraint, backfill, seed extension (DEC-004), the aggregate queries with plant-timezone bucketing, and their index support.
