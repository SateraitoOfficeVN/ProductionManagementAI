...
| # | Milestone / step | Depends on | Skill used | Deliverable | Verification method | Outcome |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | Branch `feature/WI-003-production-order-list` (worktree, as WI-002 used) | none | — | branch | `master` left clean; work happens on the branch | |
| 2 | Migrations: indexes and `pg_trgm`, then the guarded demo seed | 1 | implementation, database-design | `Migrations/*AddProductionOrderListIndexes*`, `*SeedDemoProductionOrders*` | `dotnet ef database update` against the local Compose database as the owner; both indexes present; 80 seeded rows | |
| 3 | Backend: query object, validation, contracts | 1 | implementation | `ProductionOrderListQuery`, `ProductionOrderListContracts` | `dotnet build`; covered by the unit tests in step 5 | |
| 4 | Backend: service, repository reads, controller, telemetry | 3 | implementation | `ListAsync`, `CountOrdersAsync`, `ListOrdersAsync`, query extensions, `ProductionOrdersController.List`, the new span/counter/histogram | `dotnet build` | |
| 5 | Backend unit tests | 3, 4 | testing | `Application.Tests` additions | `dotnet test` — allow-lists, date range, fragment normalization and escaping, sort mapping including the status workflow rank, the overdue rule, page arithmetic | |
| 6 | Backend integration tests | 2, 4 | testing | `Api.Tests` additions (Testcontainers) | `dotnet test` — TC-101, TC-104–TC-111, TC-114, TC-119, including an `EXPLAIN` assertion that the default-sort and fragment queries are index-backed | |
| 7 | Frontend: view state, validation, components, page, routing | 1 | implementation, screen-design | The files listed in DD-002 X-1 | `npm run lint`, `npm run build` | |
| 8 | Screen A navigation target change, with BD-001/DD-001 updated | 7 | implementation | `ProductionOrderPage`/`ProductionOrderForm`, BD-001, DD-001 | Existing Screen A tests still pass; both documents updated in the same commit | |
| 9 | Frontend unit tests | 7, 8 | testing | `tests/unit/production-orders/` additions | `npm test` — TC-102, TC-103, TC-112, TC-115, TC-116, TC-117, TC-118 (axe) | |
| 10 | E2E tests | 2, 4, 7 | testing | `tests/e2e/` additions | Compose stack plus `npx playwright test` — TC-101, TC-109, TC-113, TC-114, TC-115, TC-116, plus axe | |
| 11 | Test plan TP-003 | 5, 6, 9, 10 | testing | `work-items/WI-003/test-plan.md` | Every TC-101–TC-119 maps to a real test with its recorded result | |
| 12 | Full local verification | 2–11 | — | `evidence.md` | `dotnet build`, `dotnet test`, `npm run lint`, `npm run build`, `npm test`, Compose plus Playwright — each result recorded as actually run, with counts | |
| 13 | Gates: design-consistency, security-review, delivery | 12 | security-review, pr-review | `evidence.md` walks | Every checklist item answered; security-review covers the allow-lists, the `LIKE` escaping, the page-size cap, the role gate and the seed's environment scope | |
| 14 | Commit, push, open PR | 13 | pr-review, ci-cd | PR to `master` | CI (backend, frontend, e2e jobs) green on the PR; the diff reviewed against the design set | |
