...
While bringing up the `db` service to apply the initial migration, Claude found host port 5432 already occupied by an unrelated, pre-existing container named `PG` (`postgres:latest`) on this machine — not part of this work item, not created this session. Claude did not stop, restart, or otherwise touch that container. Instead, `deploy/compose.yaml`'s `db` service now publishes on host port 5433 by default (`POSTGRES_PORT` in `deploy/.env.example`), and `src/backend/ProductionManagementAI.Api/appsettings.Development.json`'s connection string was updated to match, so local (non-Docker) tooling and the Compose stack agree on the same port. Recorded here rather than as a numbered DEC since it's an environment-conflict workaround, not a business or architecture choice.

## Implementation note: `frontend` service host port (step 17)

Bringing up the full stack, `docker compose up -d` initially failed to start the `frontend` container: host port 8080 (the plan's implicit default) is in Windows' reserved/excluded TCP port range on this machine (`netsh interface ipv4 show excludedportrange protocol=tcp`), not merely in use by another process — no app can bind it. `deploy/compose.yaml`'s `frontend` service now publishes on host port 3000 by default (`FRONTEND_PORT` in `deploy/.env.example`). Recorded here for the same reason as the step-16 port note above: an environment-specific workaround, not a business or architecture choice.

## Implementation note: frontend unit test location (step 19)

`plan.md`'s deliverable path for step 19 is `tests/frontend/unit/*`, mirroring the `tests/backend/*` convention already used for the xUnit projects (step 13). Claude tried that literal layout first, but it doesn't transfer to the frontend: `src/frontend` is a standalone npm package with its own `node_modules`, and Vite/Vitest's module resolution walks up from the *importing file's own directory* looking for `node_modules` — a file under `tests/frontend/unit` (a sibling of `src/frontend`, not a descendant) can never reach `src/frontend/node_modules` this way. Confirmed by three successive failures: an unresolvable `setupFiles` path (`fs.allow` boundary), then a "cannot find module" error on the first real package import once that was fixed. The structurally correct fix — hoisting a shared `node_modules` via npm workspaces at the repo root — would change how every future package in this repo installs dependencies, well beyond this step's scope.

Claude instead placed the frontend unit tests at `src/frontend/tests/unit/` (inside the npm package, alongside the idiomatic Vitest layout), updated `vite.config.ts`'s `test.include`/`test.setupFiles` to point there, and left `tests/frontend/README.md` pointing to the real location with this rationale. Not logged as a numbered DEC — it's a tooling-constraint workaround, not a business or architecture choice — but recorded here since it's a visible deviation from `plan.md`'s literal deliverable path.

## Bug found and fixed: eager connection-string capture in `AddInfrastructure` (step 20)

Writing the integration test project (Testcontainers-backed Postgres + `WebApplicationFactory<Program>`) surfaced a real bug, not a test-only artifact: `AddInfrastructure` (`src/backend/ProductionManagementAI.Infrastructure/DependencyInjection.cs`) read `configuration.GetConnectionString("DefaultConnection")` once, eagerly, at service-registration time, then captured that value in a closure passed to `AddDbContext`. Under a normal `dotnet run`, all configuration sources are finalized before `Program.cs`'s own code runs, so this never mattered. Under `WebApplicationFactory`'s host-building pipeline, the sequencing between Program.cs's own registration code and `ConfigureWebHost`'s `ConfigureAppConfiguration` override is not guaranteed the same way — the eager read picked up a stale connection string from `appsettings.Development.json` (this session's own docker-compose Postgres on port 5433) instead of the test's throwaway Testcontainer, so the app under test seeded its roles/admin user into the *wrong* database, and every login attempt against the intended (empty) test database returned a generic 401.

Confirmed via a temporary diagnostic test that queried the Testcontainer's database directly and resolved `IConfiguration`/`IHostEnvironment` from the running host — environment and the final merged config were both correct, but `AddDbContext`'s closure had already captured the old value. Fixed by switching to the `AddDbContext<TContext>(Action<IServiceProvider, DbContextOptionsBuilder>)` overload, resolving `IConfiguration` from the injected `IServiceProvider` at DbContext-creation time instead of from the outer parameter at registration time — a small, generally-better pattern regardless of testing, not a testing-only workaround. Full test suite (4 unit + 4 integration) passes after the fix.

## Findings from the step-25 `ai/checklists/delivery.md` pass

Two real findings surfaced while working through the delivery checklist, both fixed within this session:

1. **Local dev password committed to `evidence.md`.** Step 18's evidence row originally quoted the actual local dev seed password used for that session's manual browser walkthrough — a direct violation of the rule Claude itself wrote into `src/backend/README.md` at step 14 ("Never write the actual password value into any committed file, including `work-items/WI-001/evidence.md`"). Redacted in the current file. The value still exists in an earlier commit in this branch's git history; since `feature/WI-001-bootstrap-skeleton` has never been pushed anywhere, this is contained to the local worktree. Claude did not rewrite git history to scrub it — amending/rebasing past commits is a more invasive git operation than this session is authorized to take without being asked, per the git-safety guidance this session operates under. If this branch is ever pushed, the user should decide whether to squash/rewrite that history first, and may want to rotate the local dev password out of caution (it only ever protected a throwaway local Postgres container, already torn down in step 23).
2. **CI actions were tag-pinned, not SHA-pinned.** `ai/checklists/delivery.md` requires "any new third-party CI action ... is pinned." `actions/checkout@v4`, `actions/setup-dotnet@v4`, and `actions/setup-node@v4` used mutable version tags, not immutable commit SHAs — the standard supply-chain-hardening distinction GitHub itself recommends. Fixed by pinning all three to their `v4` tag's current commit SHA (fetched via the public, unauthenticated GitHub API and confirmed as a direct commit reference, not an annotated-tag object), with the version kept as a trailing comment for readability.
