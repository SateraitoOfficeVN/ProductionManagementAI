...
describe('dashboardFormat (003_BD M-13, M-14, M-16–M-19)', () => {
  ...
  it('prints the lead time with one decimal and 日, — when absent (TC-212)', () => {
    expect(formatLeadTime(1.8)).toBe('1.8日')
    expect(formatLeadTime(13)).toBe('13.0日')
    expect(formatLeadTime(1)).toBe('1.0日')
    expect(formatLeadTime(null)).toBe('—')
  })

  it('adds Japanese counters and thousands separators (M-18)', () => {
    expect(formatOrders(1)).toBe('1件')
    expect(formatOrders(1234)).toBe('1,234件')
    expect(formatUnits(17922)).toBe('17,922個')
  })

  it('labels workload buckets (M-16)', () => {
    expect(bucketLabel('overdue', null, 0)).toBe('納期遅れ')
    expect(bucketLabel('week', '2026-09-22', 1)).toBe('今週')
    expect(bucketLabel('week', '2026-09-28', 2)).toBe('9/28')
    expect(bucketLabel('later', null, 9)).toBe('それ以降')
  })
  ...
  it('notes hidden rows only when there are some (M-19)', () => {
    expect(shownNote(10, 15)).toBe('15件中 10件を表示')
