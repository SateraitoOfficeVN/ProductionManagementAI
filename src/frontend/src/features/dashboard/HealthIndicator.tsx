...
const serverText: Record<ServerStatus, string> = { ok: 'OK', unreachable: 'Unreachable' }
const databaseText: Record<DatabaseStatus, string> = { ok: 'OK', unavailable: 'Unavailable', unknown: 'Unknown' }
...
// Item 26 (BD-003 HS-01–HS-05, M-20; DD-003 module 12). The status text is a polite live region, so a change is
    ...
    <div className="flex flex-wrap items-center gap-x-3.5 gap-y-1 rounded-lg border border-gray-200 bg-white px-3 py-1 text-xs text-gray-700 sm:rounded-full">
        ...
        {health.phase === 'checking' ? (
          ...
          <>
            <span className="inline-flex items-center gap-1">
              ...
              <Dot state={serverState} /> Server: {serverText[health.server]}
            </span>
            <span className="inline-flex items-center gap-1">
              <DatabaseIcon {...iconProps} size={14} className="text-gray-500" />
              <Dot state={databaseState} /> Database: {databaseText[health.database]}
            </span>
          </>
        )}
      </div>
      {checked && (
        <span className="text-gray-500 tabular-nums">
          {health.server === 'ok' ? 'Checked' : 'Last answered'} {checked}
        </span>
      )}
    </div>
  )
