...
The grants are applied by an idempotent SQL script run as the owner after migrations. No `ALTER DEFAULT PRIVILEGES` blanket grant is used: any later migration that adds a table must extend the script with that table's explicit grants. The script, the second connection string in `deploy/compose.yaml`/`.env.example` (runtime password as a new required secret, no default) and the updated migration command belong to plan revision 2 (implementation). The exact grant list is verified by an integration test that runs the app as the runtime login.
