...
DB-004 — requirements REQ-028–REQ-039 (`work-items/WI-004/brief.md`), basic design BD-003 (`docs/en/010_basic-design/BD-003-production-dashboard.md`), implements DD-003 and DD-003-API (`docs/en/020_detailed-design/`, to be written) and the REQ-033 amendment to DD-001.

This document changes one table DB-002 defined: it adds **one column and two check constraints** to `production_orders`, two partial indexes, and demo history. Everything DB-002 and DB-003 state about `products`, `production_orders` and `production_order_number_counters` stays in force and is not restated.

Physical naming follows DB-001–DB-003: `snake_case`, `timestamptz` UTC timestamps suffixed `_utc`, EF Core `pk_`/`fk_`/`ix_`/`ck_` constraint names.
...
| Table | Physical name | Purpose | Change in DB-004 |
| --- | --- | --- | --- |
| Products | `products` | Product reference data (DB-002) | none — read only, joined for product labels |
| Production orders | `production_orders` | One row per production order (DB-002) | new column `completed_at_utc`; two check constraints; two partial indexes; backfill; demo history |
| Production order number counters | `production_order_number_counters` | Order-number sequence per year (DB-002) | the seed year's counter advances past the new demo orders |

## ER diagram and relationships

Unchanged from DB-002. The dashboard traverses one relationship:
...
Why a column and not a history table: `Completed` is reachable only from `InProgress` and is terminal (BD-001 M-02), so an order is completed at most once and one timestamp holds the whole fact. DEC-003 records the trade-off.
...
`sum` and `avg` over no rows return `NULL`; the application maps `NULL` sums to 0 and a `NULL` average (with `window_count = 0`) to "—" (BD-003 M-13, M-14). Rounding and presentation are the application's, so the database returns the unrounded mean.
...
Existing indexes from DB-002 and DB-003 are unchanged. New in DB-004:

| Index name | Table | Column(s) | Type | Rationale (access pattern) |
| --- | --- | --- | --- | --- |
| `ix_production_orders_active_due_date` | `production_orders` | `due_date, order_number` `WHERE status IN ('Draft', 'InProgress')` | btree, partial | Q2–Q4 read only active orders. Active orders are the open workload, which stays roughly constant while `Completed` and `Cancelled` orders accumulate forever, so over time the active set becomes a small fraction of the table. The partial index keeps these queries proportional to the open workload rather than the table's history. Its `(due_date, order_number)` order is exactly Q3's `ORDER BY`, so each attention group is read in order and stops after 10 rows. DB-003's full `ix_production_orders_due_date_order_number` would serve Q3 too, but it walks every completed and cancelled order with an old due date before reaching the first overdue active one |
...
| --- | --- | --- |
| Dashboard snapshot (Q1–Q6, FN-017) | One explicit transaction, `BEGIN ISOLATION LEVEL REPEATABLE READ READ ONLY` … `COMMIT` (DEC-015) | In PostgreSQL, `REPEATABLE READ` gives every statement in the transaction the same snapshot, taken at the first statement. So all seven statements see one consistent state of `production_orders`, and BD-003's cross-checks hold: the tiles sum to the total, and the workload bars sum to Draft + In progress. A read-only transaction never raises a serialization failure at this level, so there is no retry path. It takes no locks beyond `ACCESS SHARE` and blocks no writer |
| Complete an order (SCR-001 save, FN-021) | The existing single `SaveChangesAsync` of DD-001-FN UpdateAsync | Unchanged: the `xmin` optimistic-concurrency check (WI-002 DEC-010, DEC-014). `completed_at_utc` is one more column in the same `UPDATE`, so the completion time and the status change commit together or not at all |
...
As in DB-003 (WI-003 DEC-011), every date is **relative to the moment the migration runs**. Everything else is a constant: ids, products, quantities and the day offsets.
...
An order finished well before its due date is plausible — and it is on time (BD-003 D-07).
...
Same sizing assumption as DB-003: about 15,000 orders a year. The active set (the open workload) stays in the hundreds to low thousands; completions in any 12-week window number about 3,500 at most. The demo database holds 124.
...
- **Migration type:** additive. `CREATE INDEX CONCURRENTLY IF NOT EXISTS` for each, via `migrationBuilder.Sql(..., suppressTransaction: true)`, exactly as DB-003's index migration.
...
- **Screen B impact:** the demo database grows from 80 to 124 orders. Screen B has no new behavior, but WI-003's documents and tests that state seeded figures — DB-003's seed table, and tests asserting the 80-row total, the status spread or the `00081` next number — are updated in plan revision 2 to the new figures. Its tests that assert relative dates are unaffected, because no seeded active order's due date changes.
...
| Active orders due beyond the 8-week look-ahead in the seed | add 4; none | add 4 | decided — DEC-014 (Claude; open to user objection at DB-004 review) |
