...
Navigation moves into a navbar in the shared application header, on every authenticated screen (DEC-016). It is specified once, in "Shared application header" below, and BD-001 and BD-002 refer to it.
...
| FN-025 | Navigate the application | The shared header's navbar: Dashboard, Production orders, New production order, current entry marked, a menu on SP | REQ-040 |
...
| --- | --- | --- | --- |
| HS-01 | Server | OK | The health endpoint answered 200 within 5 seconds |
| HS-02 | Server | Unreachable | No answer within 5 seconds, a network failure, or a 5xx |
| HS-03 | Database | OK | The server reports that a trivial database round trip (`SELECT 1`) completed within 2 seconds |
| HS-04 | Database | Unavailable | The server reports that the round trip failed or exceeded 2 seconds |
| HS-05 | Database | Unknown | The server was unreachable (HS-02), so the database's status cannot be known — never the last good value |
...
1. User activates **Expand** on a chart (item 27) → system shows the chart enlarged in a full-window view with its table alongside, from the snapshot already loaded (FN-026).
2. User activates **Restore** or presses Escape → the dashboard returns with focus on the same Expand control.
...
| --- | --- | --- | --- |
| SCR-003 | Production Dashboard | `/` — after login (WI-001's login navigates to `/`); the navbar's **Dashboard** entry and the app name on every screen; the "Home" breadcrumb on SCR-001 and SCR-002 | Navbar **Production orders** → SCR-002 `/production-orders`. Navbar **New production order** → SCR-001 create mode `/production-orders/new`. Session expired / 401 → `/login`. No widget navigates anywhere (DEC-006) |

Screen transition:

```
                 navbar (every authenticated screen): Dashboard | Production orders | New production order
                        |                         |                         |
[/login] --login OK--> [SCR-003 dashboard /]  [SCR-002 list]  <-->  [SCR-001 create/edit]
                           ^                                                  |
                           +------ app name / "Home" breadcrumb / navbar -----+

Any 401 on SCR-003 --> [/login]
```

SCR-003 takes over the route of the placeholder home page. The home page's two entry points — "Production orders" and "New production order" — move into the navbar, together with "Dashboard", on every authenticated screen (DEC-016, superseding DEC-012's page actions). SCR-001's and SCR-002's own behavior is unchanged; their breadcrumbs stay (DEC-021), and their "Home" and the header's app-name link target `/`, which is now the dashboard.

## Shared application header

The header (`AppHeader`) is shared by SCR-001, SCR-002 and SCR-003 and is not shown on `/login`. It is specified here once; BD-001 and BD-002 refer to this section.
...
| (H-1) PMAI      (H-4) [Menu] |
+------------------------------+
| (H-5) Dashboard              |   <- panel, shown when Menu is open
|       Production orders      |
...
| H-2 | Navbar entries | A `<nav aria-label="Main">` with three links: **Dashboard** (`/`), **Production orders** (`/production-orders`), **New production order** (`/production-orders/new`, styled as the primary action). The entry for the current screen is marked visually and with `aria-current="page"`: Dashboard on `/`; Production orders on `/production-orders` and on an order's edit route `/production-orders/{id}`; New production order on `/production-orders/new` |
| H-3 | Signed-in user + Sign out | Unchanged (WI-001) |
| H-4 | Menu button (SP) | Replaces H-2 and H-3 below `sm`; a `<button aria-expanded aria-controls>` labelled "Menu" |
| H-5 | Menu panel (SP) | The same three links, then the user and Sign out, stacked; closes on a link activation, on Escape (focus returns to H-4) and on navigation |
...
The navbar changes no behavior of any screen. Note: SCR-001's unsaved-change confirmation (BD-001 E-07/E-09) is attached to its Cancel button only, so until WI-004 a breadcrumb link left an edited form without asking. DEC-022 extends that confirmation to every in-app link, the navbar's included (BD-001 v7 E-07a).
...
| 2 | API base path | One read-only dashboard endpoint (`/api/dashboard`) and one health endpoint (`/api/system/health`); exact contracts in DD-003-API | DEC-011, DEC-017, DEC-020 |
...
```
+--------------------------------------------------------------------------------+
| (1) ProductionManagementAI  Dashboard | Production orders | [+ New]  (2) user   |
+--------------------------------------------------------------------------------+
| (3) Dashboard                     (26) Server ● OK · Database ● OK · 14:05:30   |
...
| |  3 orders / 3,097 | | 24 orders / 17,922| |  77%  23 of 30 | |  13.7 days   | |
| +-------------------+ +-------------------+ +----------------+ | (30 orders)  | |
|                                                               +--------------+ |
| +--------------------------------------------+ +-----------------------------+ |
...
| +--------------------------------------------+ +-----------------------------+ |
| +--------------------------------------+ +-------------------------------------+ |
| | (20) Open workload by due week  (27)⤢| | (23) Completed per week, 12 wk  (27)⤢| |
| |  ██                                  | |              ▇                      | |
| |  ██ ▇▇ ██ ▅▅ ▃▃ ▃▃ ▂▂ ▂▂ ▁▁ ▅▅        | |  ▂ ▃ ▁ ▅ ▂ ▃ ▆ ▃ ▇ ▅ ▆ ▂              | |
| |  Ovd W0 W1 W2 W3 W4 W5 W6 W7 Later   | |  07-06 …              This week      | |
...
| --- | --- | --- |
| 1 | App header with navbar | Shared application header (H-1–H-5); the Dashboard entry is marked current |
| 2 | Signed-in user + Sign out | Shared header (H-3) |
...
| 26 | Health indicator | Beside the heading: server status, database status, last-check time (HS-01–HS-05, M-20). Checking for the first time shows "Checking…" |
| 27 | Expand (per chart) | Icon button with the accessible name "Expand {chart title}" in each chart's heading row |
| 28 | Maximized chart view | Full-window dialog: chart title, the chart enlarged to the window width, its table alongside (beside it at `lg`, below it otherwise), and **Restore** (29) |
| 29 | Restore | Closes item 28; Escape does the same |
...
|      within its card)(21)(27)|
| (23) Trend chart (24)(27)    |
+------------------------------+
```

Same item numbers as PC. Differences: the navbar collapses behind **Menu** (H-4, H-5); the health indicator wraps under the snapshot time; the maximized chart view (28) fills the screen, with its table below the chart; tiles flow two per row; the attention-list rows become one card per order; widgets stack in one column in the order status tiles → delivery tiles → attention list → top products → workload → trend; a chart wider than the screen scrolls horizontally inside its own card, never the page.
...
| 26 | Health indicator | `health` | label (status, live region) | O | 2 statuses + time | — | "Checking…" | — | Always, except in the forbidden state | FN-024 | HS-01–HS-05; §4 M-20; §6 E-26–E-28 |
| 27 | Expand | — | icon button | I | — | — | — | — | Per chart, when figures are shown | — | §6 E-24 |
| 28 | Maximized chart view | — | modal dialog | O | chart + table | — | Closed | — | After E-24 | Same snapshot as the card | §6 E-24, E-25 |
| 29 | Restore | — | button | I | — | — | — | — | In item 28 | — | §6 E-25 |
...
| E-24 | (27) Expand | click / Enter / Space | Open item 28 as a modal dialog showing that chart and its table from the snapshot already loaded — no request; focus moves into the dialog (to Restore) and cannot leave it while open | REQ-042, DEC-018 |
| E-25 | (29) Restore / Escape | click / Enter / Escape | Close item 28; focus returns to the Expand control that opened it | REQ-042 |
| E-26 | (26) Health — first check | screen load | Show "Checking…"; run the check (FN-024); render HS-01–HS-05 | REQ-041 |
...
| Exception — load failure | Server, database or network error | Nothing shown as figures | Error banner MSG-E013 with **Retry**; the navbar still usable; the health indicator shows which component is failing |
| Exception — database down while the dashboard is open | The poll's database check fails | Figures already shown stay, labelled by their snapshot time | Indicator "Database: Unavailable"; the next Retry or reload may fail with MSG-E013 |
| Exception — server unreachable while open | The poll gets no answer or a 5xx | Figures already shown stay | Indicator "Server: Unreachable · Database: Unknown" |
...
| Where the placeholder home's two links go | DD-003-SPD | answered in decisions.md (DEC-012), superseded by DEC-016 — a navbar in the shared header on every screen |
| Health indicator: states, interval, timeout, endpoint | DD-003-API, DD-003-FN | answered in decisions.md (DEC-017, DEC-020) |
| Whether the health poll renews the session | DD-003-FN | answered in decisions.md (DEC-019) — never |
| Whether SCR-001/SCR-002 keep their breadcrumbs next to the navbar | BD-001, BD-002 | answered in decisions.md (DEC-021) — kept |
| How a chart goes full screen | DD-003-SPD | answered in decisions.md (DEC-018) — full-window modal dialog |
