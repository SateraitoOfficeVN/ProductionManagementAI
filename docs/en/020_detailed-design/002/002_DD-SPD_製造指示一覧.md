...
002_DD-SPD — elaborates 002_DD, implements 002_BD, requirements REQ-020–REQ-027.

## Document control (改版履歴)

| Field | Value |
| --- | --- |
| Document ID | 002_DD-SPD |
...
| 2 | 2026-09-22 | Claude (for ThanhTN) | WI-004 (DEC-016): the page renders inside the shared `AppHeader`, which now contains the navbar (`AppNavbar`, designed in 003_DD module 10 and 003_DD-SPD §8; breadcrumb links become `GuardedLink`, 003_DD module 11). No processing step of this screen changes |
| 3 | 2026-09-23 | Claude (for ThanhTN) | WI-005: the UI is Japanese. Page title, button names, paging text and formatting helpers updated to the Japanese catalog (WI-005 DEC-003, DEC-006, DEC-007). No field, rule, API or processing changes |
...
| 7 | `ProductionOrdersController.List` — request boundary | Auth, query binding and validation, result → HTTP | Server steps are in 002_DD-FN |

### Reference documents

| No | Document | Purpose / use | Notes |
| --- | --- | --- | --- |
| 1 | 002_DD | Items, states, message catalog, module list | Parent document |
| 2 | 002_DD-API | The endpoint contract called below | |
| 3 | 002_DD-FN | Server-side steps (`ListAsync`, repository methods) | |
| 4 | 002_BD | Events E-10–E-19, validation V-09–V-13, states | |
| 5 | 001_DD-SPD | SCR-001's processing, reached from this screen | Navigation target |
...
| 3 | `api.ts` (`listOrders`, `listProducts`) | Typed wrappers over `apiClient` | 002_DD module 10 |
| 4 | `ProductionOrderFilters`, `ProductionOrderTable`, `ListPagination`, `MessageBanner` | Rendered children | `MessageBanner` reused from 001_DD |
...
| 6 | Set `document.title` to `製造指示一覧 — ProductionManagementAI` (Production orders) | — | — | — |
...
| Field | Value |
| --- | --- |
| Detail | Hook owning the parse/serialize of the nine URL parameters (002_BD 0-3) |
| Created by / date | Claude / 2026-09-22 |
| Last modified by / date | — |
...
| Field | Value |
| --- | --- |
| Detail | Filter panel (items 7–13), a real `<form>` submitted by **検索** (Search) |
| Created by / date | Claude / 2026-09-22 |
| Last modified by / date | — |
...
| No | Name | Overview | Notes |
| --- | --- | --- | --- |
| 1 | `validateFilters` | Client-side V-09, V-10 | 002_DD module 5 |
...
| 3 | **検索** (Search) (click or Enter in the panel) → run `validateFilters` | Invalid → show inline errors, move focus to the first invalid field, send no query and leave the rows as they are | §002_DD module 5 | Field errors (MSG-E015/E016/E017) |
| 4 | Valid → publish the draft to the view state with `page = 1`, keeping sort and page size | — | §2 `setView(push)` | P-10 queries |
| 5 | **クリア** (Clear) (item 12) or **絞り込みをクリア** (Clear filters) (item 25) → reset the draft to empty, publish with `page = 1` | Disabled when no filter is set | §2 | P-10 queries the unfiltered list |
...
| 2 | `statusLabels`, `labels`; `formatDate`, `formatTimestamp`, `formatNumber` | Status label (M-05), other texts, and date and number formatting (M-10) | `messages.ts`; `src/lib/format.ts` (WI-005) |
...
| Step | Description | Branch / condition | Calls | Result (state / redirect / render) |
| --- | --- | --- | --- | --- |
| 1 | `lastPage = max(1, ceil(total / pageSize))`; render 「{page} / {lastPage} ページ」 (Page {page} of {lastPage}) and the M-09 summary | `total = 0` → the summary is replaced by the empty or no-match state | — | — |
| 2 | **‹ 前へ** / **次へ ›** (Previous / Next), named 「前のページ」 / 「次のページ」 for assistive technology | Disabled (not hidden) at page 1 and at `lastPage`; a page past `lastPage` still renders the controls so the user can go back | §2 `setView(push)` | P-10 queries |
...
| 2 | 200 with `total = 0` | No filter set → `empty` (MSG-I003 + **+ 新規製造指示** (New production order)); any filter set → `noMatch` (MSG-I004 + **絞り込みをクリア** (Clear filters)) | — | Filter panel keeps its values in both cases |
...
| 7 | Network failure or 5xx | — | — | `error` banner MSG-E013 with **再試行** (Retry) (E-18), which re-runs the same query unchanged |
...
Processing overview: bind the query string, validate it into a typed query object, delegate, and map the result to HTTP. No business rule and no query composition lives here — those are in 002_DD-FN.

**Used components / services**

| No | Name | Overview | Notes |
| --- | --- | --- | --- |
| 1 | `ProductionOrderListQuery.TryCreate` | Validation and normalization | 002_DD module 5 |
| 2 | `ProductionOrderService.ListAsync` | The query use case | 002_DD-FN §1 |
| 3 | `ProductionOrderProblems.ToActionResult` | `Result<T>` → HTTP, RFC 9457 | Existing, 001_DD |

**Processing flow**

| Step | Description | Branch / condition | Calls | Result (state / redirect / render) |
| --- | --- | --- | --- | --- |
| 1 | Authorize | No session → 401; wrong role → 403 | policy `ProductionOrderEditor` | Empty body, as 001_DD-API |
