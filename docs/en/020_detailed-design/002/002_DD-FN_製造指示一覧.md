...
002_DD-FN — used by 002_DD and 002_DD-API, requirements REQ-020–REQ-027.

## Document control (改版履歴)

| Field | Value |
| --- | --- |
| Document ID | 002_DD-FN |
...
| 2 | 2026-09-23 | Claude (for ThanhTN) | WI-005: the UI is Japanese. The one quoted UI text gives the Japanese paging label with an English gloss (WI-005 DEC-003). No field, rule, API or processing changes |
...
| Overview | The read side of production orders: turn a validated list query into a counted page of rows. Extends the existing `ProductionOrderService` and `IProductionOrderRepository` from 001_DD-FN rather than introducing a second service, so the plant clock, telemetry and result conventions stay shared |
...
| 7 | `IPlantClock.Today` | Plant-local date (`Asia/Tokyo`) used for `isOverdue` | Existing (001_DD-FN §6), reused unchanged |
| 8 | `IProductionOrderRepository.ProductExistsAsync` | Product-filter existence check | Existing (001_DD-FN), reused unchanged |

`ListAsync` returns the same `Result<T>` type as 001_DD-FN's methods, using only the `Ok` and `Invalid` variants — a list query cannot be `NotFound`, `Conflict` or a `RuleViolation`. Unexpected exceptions propagate to the global Problem Details handler (500, MSG-E013) rather than becoming a `Result`.

### Shared utility references

| No | Name | Overview | Notes |
| --- | --- | --- | --- |
| 1 | `IProductionOrderRepository` | EF Core data access; gains the two read methods above | Existing interface (001_DD-FN) |
...
| 5 | `ProductionOrderListQuery` | The validated query object the API layer produces | Defined in 002_DD module 5; this module never sees raw query strings |
...
| 5 | Order-number pattern | `pattern` | `string?` | ≤ 24 | ListOrdersAsync, CountOrdersAsync | Already trimmed, upper-cased, escaped and wrapped as `%…%` by the API layer (002_DD-API); used with `EF.Functions.Like(o.OrderNumber, pattern, "\\")` |
| 6 | Skip / take | `skip`, `take` | `int` | — | ListOrdersAsync | `(page − 1) × pageSize`, `pageSize` |

## Response / value mapping

`ProductionOrderListItem` (field catalog in 002_DD-API) is built by `ProductionOrderListMapper.ToListItem` from the projected row:
...
| 7 | Overdue | `IsOverdue` | `bool` | — | yes | `row.DueDate < plantToday && (row.Status == Draft \|\| row.Status == InProgress)` | Computed in memory over the page's rows, after the query — never stored, never a SQL predicate (002_DB) |
...
| --- | --- | --- | --- |
| 1 | `ProductionOrderListQuery` | `query` | Already validated and normalized by the API layer (002_DD module 5) |
...
A page beyond the last one is not an error (REQ-024): step 5 simply returns no rows, and `total` still reports the real number of matches, so the client can show 「9 / 5 ページ」 (Page 9 of 5) honestly and page back.
...
Processing overview: `db.ProductionOrders.AsNoTracking()`, filters applied by §4, then `CountAsync`. No join to `products`: every filter predicate is on `production_orders`, so the count query touches one table (002_DB).
...
Processing overview: the same filtered `IQueryable`, joined to `products` for the displayed product and the product sort key, ordered by §5, then `Skip`/`Take` and an explicit projection into `ProductionOrderListRow` — `notes`, `xmin`, `order_year`, `order_seq` and `created_at_utc` are never selected (002_DB read projection).
...
| 5 | `Status` → the workflow rank expression `Draft=1, InProgress=2, Completed=3, Cancelled=4`, not the stored string (002_DB sort-key mapping) | — |
...
Processing overview: a pure function — same row plus same date always gives the same item, which is what makes the overdue rule unit-testable without a database (002_BD M-08, FN-013).
...
Extends the existing `ProductionManagementAI.ProductionOrders` `ActivitySource`/`Meter` (001_DD-FN) rather than adding a second one, per `ai/rules/backend.md`.
...
| Metric | `pmai.production_orders.listed` | Counter | `outcome` = `success` \| `validation_failed` \| `error`, reusing 001_DD-FN's outcome names |
...
None. The behavior here follows DEC-002 (paging and sort), DEC-005 (multi-select status), DEC-010 (fragment matching) and 002_DB's query design; the plant clock and the `Result<T>`/telemetry conventions are inherited unchanged from 001_DD-FN.
