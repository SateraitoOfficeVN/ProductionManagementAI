...
001_DD-SPD — elaborates 001_DD, implements 001_BD (SCR-001), requirements REQ-010–REQ-019.

## Document control (改版履歴)

| Field | Value |
| --- | --- |
| Document ID | 001_DD-SPD |
...
| --- | --- | --- | --- |
| 1 | 2026-09-18 | Claude (for ThanhTN) | Initial creation. Moves 001_DD revision 1's processing flows P-01–P-05 into this document and splits them per component |
| 2 | 2026-09-22 | Claude (for ThanhTN) | WI-004 (DEC-016): the page renders inside the shared `AppHeader`, which now contains the navbar (`AppNavbar`, designed in 003_DD module 10 and 003_DD-SPD §8; breadcrumb links become `GuardedLink`, 003_DD module 11). No processing step of this screen changes. WI-004 DEC-022: the form registers its dirty state with the shared `NavigationGuard` (003_DD module 11, 003_DD-SPD §9), so navbar, breadcrumb and app-name links open the existing discard dialog with a pending destination (001_BD v7 E-07a); Discard navigates there |
| 3 | 2026-09-23 | Claude (for ThanhTN) | WI-005: the UI is Japanese. Page titles, button names and the dialog title quote the Japanese text with an English gloss (WI-005 DEC-003). No field, rule, API or processing changes |
...
| 5 | `ProductionOrdersController` / `ProductsController` — request boundary | Auth, content type, dispatch to the service, result → HTTP | Server steps are in 001_DD-FN |

### Reference documents

| No | Document | Purpose / use | Notes |
| --- | --- | --- | --- |
| 1 | 001_DD | Items, states, message catalog, module list | Parent document |
| 2 | 001_DD-API | Endpoint contracts called below | |
| 3 | 001_DD-FN | Server-side steps (`CreateAsync`, `UpdateAsync`, …) | |
| 4 | 001_BD | Events E-01–E-09, validation V-01–V-08 | |
...
Processing overview: decide the mode from the route, apply the client-side role gate, fetch the data, then either render the form or a full-page panel. Runs on mount and on **再読み込み** (Reload) / **Try again**.
...
| 2 | `api.ts` (`listProducts`, `getOrder`) | Typed wrappers over `apiClient` | 001_DD module 10 |
...
| 7 | Build `initialValues`: from the order (edit) or empty with status `Draft` (create); set title `製造指示 {orderNumber} — ProductionManagementAI` (Production order {orderNumber}) in edit mode; create mode's title is `新規製造指示 — ProductionManagementAI` | — | — | `ready` |
...
Processing overview: validate on blur for early feedback, validate everything on Save, then send one request. The server repeats every check (001_DD-FN), so the client checks are advisory. After a successful update the form re-bases on the server response, so `isDirty` becomes false and the lock/status options reflect the new status.
...
| 2 | `messages.ts` | Message ID → text | 001_DD catalog |
| 3 | `api.ts` (`createOrder`, `updateOrder`) | POST/PUT wrappers | 001_DD-API §2, §4 |
...
| 3 | Set `saving`: Save disabled, label 「保存中…」 (Saving…) | — | — | Saving state |
...
| No | Name | Overview | Notes |
| --- | --- | --- | --- |
| 1 | `apiClient` `ApiError` | `{ status, problem? }` | 001_DD module 10 |
...
| 5 | 409 `MSG-E009` | — | — | Error banner with **再読み込み** (Reload) → P-01 (discards local edits) |
...
| 2 | Open the dialog | `isDirty` true | `dialog.showModal()` | Dialog 「変更を破棄しますか？」 (Discard your changes?); focus on **編集を続ける** (Keep editing) |
| 3 | **破棄** (Discard) | — | `dialog.close()`, `navigate('/')` | Home; nothing saved |
| 4 | **編集を続ける** (Keep editing) or Escape (`cancel` event) | — | `dialog.close()` | Form with all values; focus returns to 「キャンセル」 (Cancel) |

### 5. ProductionOrdersController / ProductsController — request boundary

| Field | Value |
| --- | --- |
| Detail | HTTP entry points for 001_DD-API; no business logic |
| Created by / date | Claude / 2026-09-18 |
| Last modified by / date | — |

Processing overview: the ASP.NET Core pipeline authenticates and authorizes, then checks the content type. The action calls one service method and maps its `Result` to HTTP. All business checks happen in 001_DD-FN.
...
| 3 | `ProductionOrderService` | Use cases | 001_DD-FN |
