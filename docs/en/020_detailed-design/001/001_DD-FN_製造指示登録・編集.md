...
001_DD-FN — used by 001_DD and 001_DD-API (and later by Screen B's list DD), requirements REQ-010–REQ-018.

## Document control (改版履歴)

| Field | Value |
| --- | --- |
| Document ID | 001_DD-FN |
...
| --- | --- | --- | --- |
| 1 | 2026-09-18 | Claude (for ThanhTN) | Initial creation. Moves 001_DD revision 1's modules 3–5 (service, plant clock, order-number issuer) into this document |
...
| 2 | `IProductionOrderRepository` | EF Core data access: `ListProductsAsync`, `ProductExistsAsync`, `FindAsync(id, tracked)`, `Add`, `SaveChangesAsync`, `BeginTransactionAsync` | Infrastructure; single-table queries with column projections (001_DD "Database and transaction mapping") |
| 3 | `ProductionOrderTelemetry` | `ActivitySource` + `Meter` named `ProductionManagementAI.ProductionOrders` | 001_DD "Observability" |
...
`ProductionOrderResponse` (field catalog in 001_DD-API) is built by `ProductionOrderMapper.ToResponse`:
...
Processing overview: collect every field error before touching the database. Then run the data-dependent checks. Then issue the number and insert inside one transaction, so a failed insert doesn't consume a number (001_DB).
...
| 3 | Value checks: `ValidateQuantity` (MSG-E003 / MSG-E010), `NormalizeNotes` (trim, empty → null) + `ValidateNotes` (MSG-E006) | `ProductionOrder` static validators (001_DD module 1) |
...
| 7 | `order.Update(productId, quantity, dueDate, notes, status, utcNow)`; catch `DomainRuleViolation(code)` → `RuleViolation(code)` (`updated{outcome=rule_violation}`, log `ProductionOrderRuleViolated`). The entity is unchanged when it throws. On `InProgress → Completed` the entity also sets `CompletedAtUtc = utcNow` (001_DD module 1 step 5, WI-004 REQ-033); if a later step fails, nothing is saved, so no completion time is recorded | Domain (001_DD module 1) |
...
| Field | Value |
| --- | --- |
| Description | Returns the next `order_seq` for a year using 001_DB's counter upsert, inside the caller's transaction (DEC-013) |
...
| 2 | Run the upsert, return `last_seq` | 001_DB `production_order_number_counters` |
