...
001_DD-API — supports 001_DD; handlers designed in 001_DD-FN; requirements REQ-010–REQ-018.

## Document control (改版履歴)

| Field | Value |
| --- | --- |
| Document ID | 001_DD-API |
...
| 3 | 2026-09-23 | Claude (for ThanhTN) | WI-005: the UI is Japanese. The example product name is a Japanese automobile part (WI-005 DEC-002). Problem Details `title` texts stay English: they are API text for developers, never shown in the UI, which renders the message ID from `code`. No field, rule, API or processing changes |
...
- **Auth:** cookie session (0002_ADR) plus policy `ProductionOrderEditor` = role `Admin` or `Operator` (DEC-001). No session → 401 with an empty body. Session without the role → 403 with an empty body. This is the existing WI-001 cookie-event behavior, kept unchanged.
...
```
  ...
  `errors` values are message IDs from 001_DD's catalog; the client maps them to text, so wording lives in one place. Stack traces and exception messages are never included. JSON binding failures (e.g. `"quantity": "abc"`, a malformed date) are mapped to the same shape by a custom `InvalidModelStateResponseFactory`: framework keys like `$.quantity` become `quantity`, and the framework messages become the field's message ID. Framework text is never returned.
...
| 3 | Name | `name` | string | 200 | yes | yes | — | `ドライブシャフト` | | |
...
Processing overview: server side of P-02. Method-level design is in 001_DD-FN §3 `CreateAsync`; the controller boundary is in 001_DD-SPD §5.
...
| 4 | Transaction: `IOrderNumberIssuer.NextAsync(IPlantClock.CurrentYear)` → `ProductionOrder.Create` → insert → commit | 001_DB counter upsert |
...
Processing overview: server side of P-03, method-level design in 001_DD-FN §4 `UpdateAsync`. The check order is fixed: 400 (shape) → 404 → 409 (version) → 422 (Domain rules) → 400 (data-dependent) → 409 (race at save).
...
Problem `type` URIs: `urn:pmai:problem:validation`, `…:not-found`, `…:conflict`, `…:rule-violation`, `…:internal`. The top-level `code` is `VALIDATION` for 400 (details in `errors`) and the 001_DD message ID for every other error (404 `MSG-E011`, 409 `MSG-E009`, 422 `MSG-E007`/`MSG-E008`, 500 `MSG-E013`). 401/403/415 keep their framework bodies.
