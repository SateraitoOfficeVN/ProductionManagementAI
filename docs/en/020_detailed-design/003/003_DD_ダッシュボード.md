...
| --- | --- | --- | --- |
| 1 | 003_DD-API_ダッシュボード.md | api-design | `GET /api/dashboard` (API-DSH-01) and `GET /api/system/health` (API-SYS-01): response catalogs, error codes, caching |
| 2 | 003_DD-FN_ダッシュボード.md | function-design | `DashboardService.GetSnapshotAsync`, `DashboardWindow.For`, `IDashboardReader.ReadAsync`, `DashboardMapper.ToResponse`, `IPlantClock` additions, `SystemHealthService.CheckAsync`, the session-renewal rule, observability |
| 3 | 003_DD-SPD_ダッシュボード.md | screen-processing-design | P-20 to P-29 plus the two controller request boundaries |
