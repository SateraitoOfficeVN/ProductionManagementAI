...
- `frontend.nginx.conf` — serves the built frontend and reverse-proxies `/api/` to the `backend` service, so the browser only ever talks to one origin (ADR-0002: no CORS needed).
