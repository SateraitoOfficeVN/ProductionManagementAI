<!-- Function / Module Design (機能設計) template, a companion to detailed-design.md, based on the reference workbook at ai/templates/example/DD/Functions/90-11-02.機能設計_customer_inc.xlsx. Use it for a backend module/service that is shared/reused across more than one screen or DD (e.g. a domain service, a shared Application-layer helper) — logic owned by a single screen belongs directly in that screen's detailed-design.md "Module design" section instead. Copy into the relevant work item or docs/en/020_detailed-design area; replace {bracketed} prompts with task-specific facts, or "Not applicable" with a reason. Do not fabricate results or approval.
