<!-- API Specification Design (API仕様設計) template, a companion to detailed-design.md, based on the reference workbook at ai/templates/example/DD/API/70-00-02.API仕様設計_会員情報.xlsx. Use it when an API is substantial or reused enough (called by more than one screen, or with several operations/methods) to warrant its own design document rather than being folded into detailed-design.md's brief "APIs used" pointer table. Copy into the relevant work item or docs/en/020_detailed-design area, next to the detailed-design.md file(s) that reference it; replace {bracketed} prompts with task-specific facts, or "Not applicable" with a reason. Do not fabricate results or approval.
